class ShenShaCalculator:
    ...
    def __init__(self, base_result):
        ...
        self.palace_dizhi_map = {
            1: '子', 2: '未', 3: '卯',
            4: '辰', 5: '未', 6: '戌',
            7: '酉', 8: '丑', 9: '午'
        }
        # 初始化时计算所有神煞值
        self.shensha_values = self._calculate_shensha_values()

    def _calculate_shensha_values(self):
        """计算所有神煞的值（不涉及宫位）"""
        # 确保基础数据存在

        # 解析基础数据
        year_ganzhi = self.base_result['四柱']['年柱']
        month_ganzhi = self.base_result['四柱']['月柱']
        day_ganzhi = self.base_result['四柱']['日柱']
        hour_ganzhi = self.base_result['四柱']['时柱']

        # 设置计算属性
        self.year_gan = year_ganzhi[0]
        self.year_zhi = year_ganzhi[1]
        self.month_zhi = month_ganzhi[1] if month_ganzhi else ''
        self.day_gan = day_ganzhi[0] if day_ganzhi else ''
        self.day_zhi = day_ganzhi[1] if day_ganzhi else ''
        self.hour_zhi = hour_ganzhi[1] if hour_ganzhi else ''
    ...
    def calculate_all_shensha(self):
        """计算所有宫位的神煞并存储到base_result"""
        shensha_result = {}
        for palace in range(1, 10):
            dizhi = self.palace_dizhi_map[palace]
            shensha_list = self.get_palace_shensha(dizhi)
            shensha_result[palace] = shensha_list

        # 存储到base_result
        self.base_result['神煞分布'] = shensha_result
    ...
    def get_palace_shensha(self, palace_dizhi):
        ...
        for name, value in self.shensha_values.items():
            if isinstance(value, list):
                if palace_dizhi in value:
                    shensha_list.append(name)
            elif isinstance(value, str) and palace_dizhi == value:
                shensha_list.append(name)
            elif isinstance(value, bool) and value:
                # 特殊处理布尔类型神煞（如孤鸾煞）
                # 这类神煞通常不属于特定宫位，可以考虑单独存储
                pass
